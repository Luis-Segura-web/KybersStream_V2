...
class GetSeriesCategoriesUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Result<List<Category>>> = flow {
        try {
            ...
            val cachedCategories = databaseCacheManager.getCachedXtreamCategories(userHash, "series")
            ...
            when (val result = xtreamRepository.getSeriesCategories()) {
                is XtreamResult.Success -> {
                    // Cache the data
                    databaseCacheManager.cacheXtreamCategories(result.data, "series", userHash)
